...
Welcome to TheChosenDownloader, a collection of Python scripts designed to scrape video data from a GraphQL API, parse the data, and store it in a SQLite database. It also includes scripts to query the database and download the video, audio, and subtitle streams. The scripts are designed to be used in sequence, with each script building on the data generated by the previous one.

TheChosenDownloader is perfect for users who want to download and organize video content in a structured and automated way. It provides flexibility in terms of video quality, audio language, and subtitle language, and organizes the downloaded content by season and episode.
...
## scrape.py
This script is used to scrape video data from a GraphQL API, parse the data, and store it in a SQLite database. It's written in Python and uses the `requests`, `sqlite3`, `m3u8`, `time`, and `re` libraries.

### How it works
The script starts by connecting to a SQLite database named `chosen_links.db` and creating a table named `links` if it doesn't already exist.

It then defines a function `parse_m3u8` that takes a URL, season number, episode number, and title as arguments. This function loads the M3U8 file from the URL and parses it, extracting the media URLs and playlist URLs and inserting them into the `links` table in the database.

The script sends a POST request to a GraphQL API and retrieves a list of videos. It filters this list to include only full episodes and sorts them by title.
